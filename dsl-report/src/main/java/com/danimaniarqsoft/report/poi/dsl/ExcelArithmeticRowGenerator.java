...
public class ExcelArithmeticRowGenerator {
    ...
    public static void createArithmeticRow(ExcelSheetParams params,
            CellState cellState) {
        char[] cellLetter = new char[] { (char) (LetterHolder.ASCII_CAPITAL_A + params
                .getOffset()) };
        ...
        for (int i = params.getOffset(); i < params.getTablaAncho(); i++) {
            ...
            cellState.createCellFormula(buildSumFormula(letterHolder,
                    params.getMinSum(), params.getMaxNum()));
            cellLetter = ExcelArithmeticRowGenerator
                    .calculateNextSequence(cellLetter);
        }
    }

    public static void createAverageRow(ExcelSheetParams params,
            CellState cellState) {
        ...
        for (int i = params.getOffset(); i < params.getTablaAncho(); i++) {
            letterHolder.setLength(0);
            letterHolder.append(cellLetter);
            cellState.createCellFormula(buildAverageFormula(letterHolder,
                    params.getMinSum(), params.getMaxNum()));
            cellLetter = ExcelArithmeticRowGenerator
                    .calculateNextSequence(cellLetter);
        }
    ...
    private static String buildSumFormula(StringBuilder sb, int minSum,
            int maxSum) {
        StringBuilder formulaBuilder = new StringBuilder();
        formulaBuilder.append("SUM(");
        formulaBuilder.append(sb.toString()).append(minSum).append(":");
    ...
    private static char[] calculateNextSequence(char[] sb) {
        ...
        if (lastChar + 1 > ASCII_CAPITAL_Z) {
            return addChar(calculateNextSequence(returnSaveValue(sb)),
                    ASCII_CAPITAL_A);
        } else {
    ...
    private static char[] addChar(char[] baseSequence, char newChar) {
        ...
        char[] mergeSequence = new char[baseSequence.length
                + newCharSequence.length];
        System.arraycopy(baseSequence, 0, mergeSequence, 0, baseSequence.length);
        System.arraycopy(newCharSequence, 0, mergeSequence,
                baseSequence.length, newCharSequence.length);
