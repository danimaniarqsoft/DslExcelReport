...
public class RowState {
    ...
    public <T> RowState createRows(List<T> rows, Class<T> classWithAnnotation) {
        try {
            ExcelColumnReflection.createRowStateFromExcelAnnotation(context,
                    rows, classWithAnnotation);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return this;
    ...
    private void mergeRowArea(CellState cellState, int rowNum, int fromCellNum,
            int toCellNum) {
        ...
        context.getCurrentSheet()
                .addMergedRegion(
                        new CellRangeAddress(rowNum, rowNum, fromCellNum,
                                toCellNum - 1));
    }
